...
$.validationEngine = {
	...
	validateCall : function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
		...
		function _customRegex(caller,rules,position){		 // VALIDATE REGEX RULES
			...
			if(!pattern.test($(caller).attr('value'))){
				if(customRule == 'onlyFloatVarPrice')
				{
					temp = $(caller).attr('name');
					active = temp.split("_");
					indexAc =  parseInt(active[2])-1;
					$("#accordion").accordion('activate',indexAc);
   ...
   //do something
	...
	// dopsat
		$( "#tabsDetail" ).tabs({ selected: 0 });
		$(  "#tabsDetail").tabs( { disabled: [1, 2,3,4,5] } );
	//alert('dsa');
	//$.preventDefault();
	...
	calculatePosition : function(caller,promptText,type,ajaxed,divFormError){
		...
		if($.validationEngine.settings.promptPosition == "topLeft"){ callerTopPosition += -inputHeight -3; }
