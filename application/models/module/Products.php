	...
	public function initSearch( $sort = 'title', $sortType = 'Asc', $limitStart = 0, $limitCount = 5, $params,$view, $count = false)    
    {
        ...
        if($params['showAllV'])
        {
        	$select->join(
        			array('var' => $this->_tableNameVariants),
        			'var.id_product = cm.id',
        			array('id as variantId',  'EAN','SmartConnector','inv_title', 'skladem', 'ext_id', 'poradi','purchase_price', 'obrazky', $price, 'price2', 'discount', 'realSold','skladem','heureka-hlid')
        	);       
        }          
        else{     
        $select->join(      
        		array('var' => $this->_tableNameVariants),     
        		'var.id_product = cm.id',    
        			array('id as variantId',  'EAN','SmartConnector','inv_title', 'skladem', 'ext_id', 'poradi', 'purchase_price', 'obrazky', $price, 'price2', 'discount','ovladani','typ', 'tlak', 'objem', 'vlastnosti', 'prikon', 'hmotnost','heureka-hlid')
        );
        }
 	...
 	if($params['showFirstVariant'])
		...
		if($params['onlykavovars'])
		{
			$wc[] = " cm.parent like '%7391%' ";
			$wc[] = " cm.parent like '%74760%' ";
			$wc[] = " cm.parent like '%7234%' ";
			$select->where(implode('OR', $wc));    
		}
        ...
        if($params['znacka']){   
		 ...
		 if($params['line'] == 2){    
         	$select->where('var.secondExport  = ?', '1');    
			 $select->where('var.mainExport != ?', '1');   
         }
		 if($params['line'] == 1){    
         	$select->where('var.mainExport  = ?', '1');   
         }	    
