...
{   
	function __construct($domain = ''){
		...
		$this->_tableName = 'content_Video';
	}
    
	public function initSearch( $sort = 'id', $sortType = 'Asc', $limitStart = 0, $limitCount = 5, $root, $count = false)    
    {
		...
		if($count){    		
    		...
    		$select->from(array( 'cm' => $this->_tableName), array('n.id', 'n.title', 'n.path', 'n.parent', 'dateCreate', 'html','videoUrl','videoUrlImg')); 		  
    	}
		...
		$select->where('c_type = ?', 'content_Video');  
    ...
    public function getVideos( $sort = 'title', $sortType = 'Asc', $limitStart = 0, $limitCount = 5, $root = 0)   
    {
    ...
    public function getVideosCount( $sort = 'title', $sortType = 'Asc', $limitStart = 0, $limitCount = 5, $root = 0)   
    {   		
		return $this->db->fetchOne("SELECT count(`id`) as pocet FROM `content_Video`");	  	  
    }   
