...
class module_Voucher{ 
	...
	{
		return $this->db->fetchOne("select count(*) from `content_Product` where kod=:c ",array('c' => $code)); 
	}
	...
	public function addOrder($vid,$oid){
		...
		$data = array( 	'id_v' => $vid,
						'id_o' => $oid);
		$this->db->insert( 'voucherOrders' , $data);
		
	}


	function getVoucherByOrderId($oid){
		$select = 'SELECT id_v FROM voucherOrders WHERE id_o = '. $oid;
		$vid = $this->db->fetchOne($select);
		if(!$vid){
			return null;
		}
		return $this->getDetailVoucherById($vid);
	}

	
	function getOrders($vid)
	{
		$select = 'SELECT id_o FROM voucherOrders WHERE id_v = '. $vid;
	...
	public function initSearch( $sort = 'n.id', $sortType = 'Asc', $limitStart = 0, $limitCount = 5, $params = false)   
    {
		...
		if($params['count']){    		
    		...
    		$select->from($this->_tableName , array('id_v','title', 'code','status','type','value','id_order','valid','category','countFrom','created','numberOfUses','used','productCode','znacky'));
	...
	public function getDetailVoucherById($id){ 
		return $this->db->fetchRow("SELECT * FROM $this->_tableName WHERE id_v = ?", $id);   
	}
	...
	public function useVoucher($id){
		$uses = $this->db->fetchRow("SELECT numberOfUses,used FROM `" . $this->_tableName ."` WHERE id_v=:id", array('id' => $id));
		if($uses['numberOfUses'] > 0)
		{
			$uses['used'] += 1;
			$data = array(
				'used' => $uses['used']
			...
			if($uses['used'] >= $uses['numberOfUses'])
				$data['status'] = '2';
	...
	public function addVoucher($data){
				...
				$data['znacky'] = implode(',',$data['znacky']);
	...
	function generateCode(){
		...
		return strtoupper($pref) . floor(rand(10, 99));     
	}
	...
	function getZnackaByNodeId($id)
	{
		if(!$this->znackyCiselnik)
		{
			$znackyNode = Zend_Registry::getInstance()->tree->getNodeById(3817);
			$znacky = $znackyNode->getChildren('FOLDER');
			foreach($znacky as $znacka)
				$this->znackyCiselnik[$znacka->nodeId] = $znacka->title;
		}
		
		return $this->znackyCiselnik[$id];
	}
	...
	/**
	*	@param $orderDetails from orders->getOrderDetails
	* 	vypocita cenu se slevou pro danou objednavku a voucher
	* 	TODO sloucit na jedno misto, podobny vypocet je v Basket.php
	*/
	function getOrderPrizeWithVoucher($order, $orderDetails){
		$voucher = $this->getVoucherByOrderId($order['id']);
		if(!voucher){
			return $order['suma'];
		}
		...
		if($voucher['znacky'])
		{
			...
			foreach($znacky as $znacka)
				$znackyCiselnik[$znacka] = $this->getZnackaByNodeId($znacka);
		}

		$discountSum = $noDiscountSum = $sumAll = 0;

		foreach ($orderDetails as $oDetail) {
			$pContent = unserialize($oDetail['content']);
			$price  = $pContent->getPropertyValue('price');   
			$znacka  = $pContent->getPropertyValue('znacka');   
			$count  = $oDetail['count'];
			$kod = $pContent->getPropertyValue('kod'); 
			$sum = $price * $count;

			if($voucher['znacky'])
			{
				if(isset( $znackyCiselnik[$znacka] ))
					$discountSum += $sum;
				else
					$noDiscountSum += $sum;
			}
		...
		$dph = round($dph, 2);    
		$sumBezdph = $sum - $dph; 
		if($sumBezdph >= Zend_Registry::getInstance()->config->eshopSettings->freeDeliveryPrice){
			$delivery = 0;
		}
