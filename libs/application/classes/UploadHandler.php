...
class UploadHandler
{
    ...
    protected $error_messages = array(
        ...
        'max_file_size' => 'Soubor je příliš velký',
        'min_file_size' => 'File is too small',
        'accept_file_types' => 'Tento typ souboru není možné nahrát',
        'max_number_of_files' => 'Maximum number of files exceeded',
        'max_width' => 'Obrázek je příliš velký (široký)',
        'min_width' => 'Obrázek je příliš malý (široký)',
        'max_height' => 'Obrázek je příliš velký (vysoký)',
        'min_height' => 'Obrázek je příliš malý (vysoký)' 
    );
    ...
    function __construct($options = null, $initialize = true) {
        $this->options = array(
            ...
            'show_medium' => $options['show_medium'],
    ...
    protected function get_file_object($file_name) {
        if ($this->is_valid_file_object($file_name)) {
            ...
            $file_name = $file->name = Utils::generatePathNameSimple($file_name);  
    ...
    protected function trim_file_name($name, $type, $index, $content_range) {
        ...
        // Also remove control characters and spaces (\x00..\x20) around the filename:
         ...
         $name = Utils::generatePathNameSimple($name, '.-');   
        ...
        if (strpos($name, '.') === false &&
            preg_match('/^image\/(gif|jpe?g|png)/i', $type, $matches)) {
            $name .= '.'.$matches[1];
        }
    ...
    public function post($print_response = true) {
        ...
        /* 
	    [name] => 045-news-sk-0404_18 (1).jpg
	    [size] => 15796
	    [type] => image/jpeg
	    [url] => /data/sharedfiles/obrazky/045-news-sk-0404_18%20%281%29.jpg
	    [thumbnail_url] => /data/sharedfiles/obrazky/thumbnail/045-news-sk-0404_18%20%281%29.jpg
	    [delete_url] => http://new.eshop-povleceni.cz/?file=045-news-sk-0404_18%20%281%29.jpg
	    [delete_type] => DELETE 
         */
        if(!isset($_SESSION['next'])){
      		$_SESSION['next'] = 1;
        } 
        $extra = unserialize($this->options['extra']);  
        foreach ($files as $i => $file) {    

        	if($_SESSION['next'] > 100){
        		$_SESSION['next'] = 1;
        	}
        	if($_SESSION['next'] <= $extra['nextId']){
        		$_SESSION['next'] = $extra['nextId'];
        	}  
        	$_SESSION['next']+= 1;  
        	
        	
        	 
        	$base = Zend_Registry::getInstance()->config->htdocsRoot; 
        	if(strpos($file->url, '.png') && Zend_Registry::getInstance()->settings->getSettingValue('disableJPGTransform') != '0' ){

	        	$asJPG = str_replace(array('.png', '.gif'), '.jpg', $file->url);
	        	$file->name = str_replace(array('.png', '.gif'), '.jpg', $file->name);
	        	$oldImage = ImageCreateFromPng($base . $file->url); 
        	...
        	$imgpath = $this->importImage($file->url, $file->origName, $this->options['folderId'], 5, $extra);
        	     
        	$file->filesnames = $extra['filesnames'];    
        	if($extra['nextId']){ // 1 file fix 
        		$file->inputId = $_SESSION['next']  ;     
        	} else { 
        		$file->inputId = ''; 
        	}
        	$file->path = $imgpath;         
        	
        	$file->showAlt = $extra['showAlt'];    
        	$file->showUrl = $extra['showUrl'];   
        	
        	$file->name = content_SFSFile::getFileWithouExtension($file->origName);         
        	 // pro editor
			if($this->options['show_medium'])
			{
				$file->path = $file->medium_url; 
	...
	function importImage($filepath, $imageName, $addTo = 2, $returnFullPath = 10, $extra){ // 3880
		...
		$view->input->fullpath = substr($filepath, strlen($this->config->sfFolder)); 	
		...
		$cm = $nm->getMetaById($nnode->nodeId);
		 
		$isz = getimagesize ($this->config->htdocsRoot . $filepath);   
		$cm['with'] = $isz[0];
		$cm['height'] = $isz[1];
		$cm['atr'] = $isz[3];
		$cm['mime'] = $isz['mime'];
		
		$nm->setMetaById($nnode->nodeId, $cm);
		
		//$filepath = str_replace("//", '/' . $nnode->nodeId . '/', $filepath);
		//pr($view->input);  
		//e($filepath); 
