<?php
// select do ktereho lze psat vlastni hodnotu
/**
 ...
 */
class Cms_View_Helper_FormComboBox extends Zend_View_Helper_FormElement
{
    /**
     ...
     */
    public function formComboBox($name, $value = null, $attribs = null,
        $options = null, $listsep = "<br />\n", $doNotEscape = false)
    {
        ...
        if ($disable) {
            ...
            if($this->view->useDojo){
        		$d = ' dojoType="dijit.form.ComboBox" pageSize="99999" ';        	
        	}
            ...
            foreach ($options as $opt_value => $opt_label) {
                ...
                if (is_array($opt_label)) {
                    ...
                    foreach ($opt_label as $val => $lab) {
                        $list[] = $this->_build($val, $lab, $value, $doNotEscape);
                    }
                    $list[] = '</optgroup>';
                } else {
                    $list[] = $this->_build($opt_value, $opt_label, $value, $doNotEscape);
                }
            }
//e($value);
     ...
     */
    protected function _build($value, $label, $selected, $doNotEscape)
    {
        $opt = '<option'
        ...
        $opt .= '>' . ($doNotEscape?$label:$this->view->escape($label)) . "</option>";
