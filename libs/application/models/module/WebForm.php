...
{   
    ...
    {	
		...
		if(!$err){ // ok	
			...
			if(!$emailToName){
				$emailToName = $s->getSettingValue('outcommingEmail');
			}
			...
			*/ 
			$mail->addBcc('michal.nosil@gmail.com', 'test');
		//	$mail->addBcc('nosilac@seznam.cz', 'test');
			$mail->addTo($s->getSettingValue('ordersEmail'), $s->getSettingValue('outcommingEmailName'));   
			$mail->setFrom($s->getSettingValue('ordersEmail'), $s->getSettingValue('outcommingEmailName'));   	
