...
class module_Slovnik 
{   
	function __construct($domain = ''){
		$this->db =  Zend_Registry::getInstance()->db;
		$this->_tableName = 'content_Slovo';
	}
	...
	public function initSearch( $parent, $sort = 'dateShow', $sortType = 'Asc', $limitStart = 0, $limitCount = 5)    
    {
		...
		if($count){    		
    		...
    		$select->from(array( 'cm' => $this->_tableName), array('n.id', 'n.title', 'n.path', 'n.parent', 'dateCreate', 'html')); 	 	  
    	}  
		...
		$select->where('c_type = ?', 'content_Slovo');  
